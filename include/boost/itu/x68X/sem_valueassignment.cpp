...
namespace x680 {
    ...
    value_atom_ptr value_atom::skip_defined(bool except_abstract) {
        if (as_defined()) {
            if (as_defined()->reff() && as_defined()->reff()->as_valueassigment()) {
                if (as_defined()->reff()->as_valueassigment()->value()) {
                    ...
                    return as_defined()->reff()->as_valueassigment()->value()->skip_defined(except_abstract);
                }
            }
            return value_atom_ptr();
        }
        return boost::static_pointer_cast<value_atom> (self());
    }
    ...
    void value_atom::resolve_vect(value_vct& vl, basic_atom_ptr holder) {
        if ((reff_resolver()) && 
                (reff_resolver()->as_type()) && 
                (reff_resolver()->as_type()->isvaluestructure())) {
            if (vl.size() == 2) {
    ...
    boost::shared_ptr<namedvalue_initer> value_atom::get_value(bool except_abstract) {
        if (as_named())
            return boost::make_shared<namedvalue_initer>(as_named()->name(), as_named()->value());
    ...
    boost::shared_ptr<namedvalue_initer_set> value_atom::get_value(bool except_abstract) {
        if ((as_struct())) {
            ...
            return boost::make_shared<namedvalue_initer_set>(rslt);
        }
    ...
    boost::shared_ptr<null_initer> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            return get_value_parent<null_initer>(except_abstract);
        } else if (as_null()) {
            return boost::make_shared<null_initer>();
        }
        return boost::shared_ptr<null_initer>();
    }
    ...
    boost::shared_ptr<int64_t> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            ...
            return boost::make_shared<int64_t>(static_cast<int64_t> (as_number()->value()));
        }
    ...
    boost::shared_ptr<std::string::value_type> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            ...
            if (tmp.size() == 1)
                return boost::make_shared<std::string::value_type>(tmp[0]);
        }
    ...
    boost::shared_ptr<std::size_t> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            ...
            return boost::make_shared<std::size_t>(static_cast<std::size_t> (as_number()->value()));
        }
    ...
    boost::shared_ptr<bool> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            ...
            return boost::make_shared<bool>(as_bool()->value());
        }
    ...
    boost::shared_ptr<double> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            ...
            return boost::make_shared<double>(as_real()->value());
        } else if (as_number()) {
            return boost::make_shared<double>(as_number()->value());
        }
    ...
    boost::shared_ptr<std::string> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            ...
            return boost::make_shared<std::string>(as_cstr()->value());
            ...
            for (value_vct::const_iterator it = lst->values().begin(); it != lst->values().end(); ++it) {
                if (*it) {
                    if ((*it)->get_value<std::string>(except_abstract)) {
                        boost::shared_ptr<std::string> sub = (*it)->get_value<std::string>(except_abstract);
    ...
    boost::shared_ptr<tuple> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            if (get_value_parent<tuple>(except_abstract))
                ...
                return boost::make_shared<tuple>(static_cast<uint8_t> (*tmp / 16), static_cast<uint8_t> (*tmp % 16));
            }
        } else if (as_list()) {
            boost::shared_ptr<num_vector> fnd = get_value<num_vector>();
            if (fnd) {
                if (fnd->size() == 2) {
                    if (((fnd->operator[](0) >= 0) && (fnd->operator[](0) <= 7)) &&
                        ...
                        return boost::make_shared<tuple>(static_cast<uint8_t> (fnd->operator[](0)),
                            static_cast<uint8_t> (fnd->operator[](1)));
                }
            }
    ...
    boost::shared_ptr<string_vector> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            ...
            boost::shared_ptr<string_vector> rslt(new string_vector());
            structvalue_atom_ptr lst = as_list();
            for (value_vct::const_iterator it = lst->values().begin(); it != lst->values().end(); ++it) {
                if (*it) {
                    if ((*it)->get_value<std::string>(except_abstract)) {
                        rslt->push_back(*((*it)->get_value<std::string>(except_abstract)));
                    } else
    ...
    boost::shared_ptr<num_vector> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            ...
            return boost::make_shared<num_vector>();
        }
        return boost::shared_ptr<num_vector>();
    }
    ...
    boost::shared_ptr<unum_vector> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            ...
            boost::shared_ptr<unum_vector> rslt = boost::make_shared<unum_vector>();
            ...
            return boost::make_shared<unum_vector>();
        }
        return boost::shared_ptr<unum_vector>();
    }
    ...
    boost::shared_ptr<bstring_initer> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            return get_value_parent<bstring_initer>(except_abstract);
        } else if (as_cstr()) {
            ...
            if (valtype() == v_bstring) {
                ...
                return boost::make_shared<bstring_initer>(rslt, cnt ? 4 : 0);
            }
        } else if (as_empty()) {
            return boost::make_shared<bstring_initer>();
        }
    ...
    boost::shared_ptr<hstring_initer> value_atom::get_value(bool except_abstract) {
        ...
        if (tmp && !(tmp->unused % 8)) {
            return boost::make_shared<hstring_initer>(tmp->str);
        }
    ...
    boost::shared_ptr<quadruple_vector> value_atom::get_value(bool except_abstract) {
        if (as_defined()) {
            return get_value_parent<quadruple_vector>(except_abstract);
            ...
            return rslt;
        } else if ((as_cstr()) && (get_value<std::string>(except_abstract))) {
            std::string tmp = *(get_value<std::string>(except_abstract));
            boost::shared_ptr<quadruple_vector> rslt(new quadruple_vector());
            for (std::string::const_iterator it = tmp.begin(); it != tmp.end(); ++it)
    ...
    void valueassignment_entity::resolve(basic_atom_ptr holder) {
        ...
        resolve_child();
        type()->resolve();
        if (value_atom_ptr tmpvalue = value()) {
