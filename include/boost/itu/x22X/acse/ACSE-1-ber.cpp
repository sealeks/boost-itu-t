...
namespace ACSE_1 {
    ...
    template<> void Associate_source_diagnostic::serialize(boost::asn1::x690::output_coder& arch) {
        switch (type()) {
            ...
            {
                ITU_T_EXPLICIT_TAG(*value<integer_type > (false, Associate_source_diagnostic_acse_service_user), 1);
                break;
            }
            case Associate_source_diagnostic_acse_service_provider:
            {
                ITU_T_EXPLICIT_TAG(*value<integer_type > (false, Associate_source_diagnostic_acse_service_provider), 2);
                break;
            }
            default:
            {
        ...
        switch (arch.test_class()) {
            ...
            case 0x80:
            {
                switch (__tag_id__) {
                    case 1:
                    {
                        if (ITU_T_EXPLICIT_TAG(*value<integer_type > (true, Associate_source_diagnostic_acse_service_user), 1)) return;
                        else free();
                        break;
                    }
                    case 2:
                    {
                        if (ITU_T_EXPLICIT_TAG(*value<integer_type > (true, Associate_source_diagnostic_acse_service_provider), 2)) return;
                        else free();
                        break;
                    }
                    default:
