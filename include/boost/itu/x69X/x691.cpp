    ...
    namespace asn1 {
        namespace x691 {
            ...
            std::size_t to_x691_cast(uint8_t val, octet_sequnce& src) {
                return to_x691_cast(static_cast<int16_t> (val), src);
            }
            ...
            static std::size_t to_x691_cast_realimpl(T val, octet_sequnce& src) {
                ...
                tmp.clear();
                std::size_t expsz = to_x691_cast(exp, tmp);
            ...
            std::size_t to_x691_cast(double val, octet_sequnce& src) {
                return to_x691_cast_realimpl<double, int64_t, DOUBLE_MANTISSA_SIZE, DOUBLE_EXPONENTA_DELT > (val, src);
            }

            template<>
            std::size_t to_x691_cast(long double val, octet_sequnce& src) {
                return to_x691_cast_realimpl<double, int64_t, DOUBLE_MANTISSA_SIZE, DOUBLE_EXPONENTA_DELT > (static_cast<double> (val), src);
            }

            template<>
            std::size_t to_x691_cast(float val, octet_sequnce& src) {
                return to_x691_cast_realimpl<float, int32_t, FLOAT_MANTISSA_SIZE, FLOAT_EXPONENTA_DELT > (val, src);
            }
            ...
            static void to_x691_castoid_impl(oidindx_type vl, octet_sequnce& src) {
                octet_sequnce tmp;
                bool first = true;
                while (vl || first) {
                    first = false;
            ...
            std::size_t to_x691_cast(const oid_type& val, octet_sequnce& src) {
                ...
                tmpval += val[1];
                to_x691_castoid_impl(tmpval, src);
                for (oid_type::const_iterator it = (val.begin() + 2); it != val.end(); ++it)
                    to_x691_castoid_impl(*it, src);
                return (src.size() - strtsz);
            }
                ...
                if (!lentype) {
                    ...
                    while (it != val.end()) {
                        ...
                        if (diff > static_cast<difference_type> (CER_STRING_MAX_SIZE - 1)) {
                            ...
                            stream.add(to_x691_cast(size_class(static_cast<std::size_t> (diff + 1))));
            ...
            std::size_t tag_x691_cast(tag& val, const mutable_sequences& src, mutable_sequences::const_iterator bit, std::size_t beg) {
                octet_sequnce s1;
            ...
            template<typename T>
            static bool from_x691_real_cast_decimal(T& vl, const octet_sequnce& val) {
                try {
            ...
            template<typename T, typename B, std::size_t MANT, std::size_t EXPB>
            static bool from_x691_real_cast_bin(T& vl, const octet_sequnce& val) {
                if (!val.empty()) {
                    ...
                    int32_t exp_b = 0;
                    if (!from_x691_cast(exp_b, octet_sequnce(val.begin() + 1, itrexen)))
                        return false;
            ...
            bool from_x691_cast(float& vl, const octet_sequnce& val) {
                if (val.empty()) {
                    ...
                    switch (val[0] & '\xC0') {
                        case 0: return from_x691_real_cast_decimal(vl, val);
                        case '\x40': return from_x691_real_cast_special(vl, val);
                        default: return from_x691_real_cast_bin<float, uint32_t, FLOAT_MANTISSA_SIZE, FLOAT_EXPONENTA_DELT > (vl, val);
                    }
                }
                return false;
            }
            ...
            bool from_x691_cast(double& vl, const octet_sequnce& val) {
                if (val.empty()) {
                    ...
                    switch (val[0] & '\xC0') {
                        case 0: return from_x691_real_cast_decimal(vl, val);
                        case '\x40': return from_x691_real_cast_special(vl, val);
                        default: return from_x691_real_cast_bin<double, uint64_t, DOUBLE_MANTISSA_SIZE, DOUBLE_EXPONENTA_DELT > (vl, val);
                    }
                }
                return false;
            }
            ...
            bool from_x691_cast(long double& vl, const octet_sequnce& val) {
                if (val.empty()) {
                    ...
                    switch (val[0] & '\xC0') {
                        ...
                        {
                            double tmp = 0;
                            if (from_x691_real_cast_bin<double, uint64_t, DOUBLE_MANTISSA_SIZE, DOUBLE_EXPONENTA_DELT > (tmp, val)) {
                                vl = tmp;
                                return true;
                            }
                        }
            ...
            bool from_x691_cast(oid_type& val, const octet_sequnce& vl) {
                ...
                oidindx_type tmp = 0;
                while (its != vl.end()) {
                    if (!from_x691_impl_cast(tmp, vl, its))
                        return false;
                    if (!fst)
                        val.push_back(tmp);
                    else {
